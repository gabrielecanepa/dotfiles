...
- **Allowed types (non-standard):** `agents`, `brew`, `chore`, `docs`, `editor`, `git`, `node`, `python`, `ruby`, `shell`. Allowed scopes (optional): `cask`, `formula`, `mas` (under `brew`), `vscode` (under `editor`), `npm` (under `node`). Do **NOT** use `feat`/`fix`/`refactor` here.
