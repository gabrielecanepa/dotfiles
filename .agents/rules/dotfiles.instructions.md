...
- npm globals are tracked by the `npm-global` plugin: the `npm` wrapper re-dumps `.npm/package.json` (synchronously, offline) after every mutating `-g` command (`npm dump` syncs, `npm fresh` updates then syncs, bare `npm i -g` reinstalls the manifest), and the `.config/nodenv/hooks/install/npm-global.bash` hook installs the manifest into every new Node version. Repo tooling still comes from the Brewfile (commitlint, oxfmt, shellcheck, shfmt) and `pnpm` from Corepack via its own install hook; never install those with npm.
