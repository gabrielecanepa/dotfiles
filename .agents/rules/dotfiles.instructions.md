...
- **Install skills with `npx skills add <owner/repo>`** (the [skills.sh](https://skills.sh) CLI), run from `~` so it writes into `.agents/skills/` and pins the entry in `.agents/.skill-lock.json`. The skills themselves are gitignored (`.agents/skills/.gitignore` is allowlist-style: `/*/` then `!`-unignore the few tracked ones); the lockfile is the tracked source of truth. Don't hand-copy a skill folder or hand-edit `.skill-lock.json`.
