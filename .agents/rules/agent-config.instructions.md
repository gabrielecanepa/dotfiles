...
- Claude auto memory is enabled (`autoMemoryEnabled` in `.claude/settings.json`): Claude writes machine-local notes under `~/.claude/projects/<project>/memory/`, and the first 200 lines or 25KB of each `MEMORY.md` load every session outside the tracked instruction stack. During agent-surface reviews, audit it with `/memory`: fold durable learnings into the tracked rules, prune the rest, and keep the directory untracked.
