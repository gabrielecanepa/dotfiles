...
- Reuse a running app before spawning one: attach to the dev server the user already has open (it holds the logged-in session a blank server lacks); start your own only when none is running. Real-browser mechanics (which tool, port reuse, session cleanup) live in `browser.instructions.md`; never build a throwaway page on an ad-hoc port to verify a feature.
