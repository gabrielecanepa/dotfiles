...
Re-asserts machine state that drifts, such as git hooks paths, clobbered symlinks, Obsidian vault links, launch agents, VS Code extensions, and npm globals.
...
- `OBSIDIAN_VAULT`: exact name of the vault folder in the iCloud Obsidian container. When it is set, `~/.vault` is linked to that folder; when unset, the vault checks are skipped.
