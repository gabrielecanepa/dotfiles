...
profile() (
  ...
  local marker_begin='# BEGIN PROFILE (generated by profile plugin)'
  ...
  _profile_write() {
    ...
    if [[ -f "$zprofile" ]]; then
      ...
      while IFS= read -r line; do
        ...
        # Drop an orphaned opening marker written by an earlier $marker_begin wording,
        # which would otherwise be preserved as hand-added content.
        [[ "$line" == '# BEGIN PROFILE (generated by'*')' ]] && continue
        lines+=("$line")
      done < "$zprofile"
    fi
  ...
  case "$1" in
    config)
      ...
      print -r -- "Run ${fg[cyan]}exec zsh${reset_color} to reload your shell with the new profile."
      return 0
      ;;
